...
module.exports = {
  ...
  get_distribution: function(richlist, stats, cb) {
    ...
    async.timesSeries(richlist.balance.length, function(i, loop) {
      const amount = richlist.balance[i].balance / 100000000;
      runningTotal += amount;
      let adjustedAmount = amount;
  
      // Cap at total supply
      if (runningTotal > stats.supply) {
        adjustedAmount -= (runningTotal - stats.supply);
        runningTotal = stats.supply;
      }
      if (adjustedAmount < 0) adjustedAmount = 0;
  
      const percentage = (adjustedAmount / stats.supply) * 100;
  
      if ((i + 1) <= 25) {
        distribution.t_1_25.percent += percentage;
        distribution.t_1_25.total += adjustedAmount;
      } else if ((i + 1) <= 50) {
        distribution.t_26_50.percent += percentage;
        distribution.t_26_50.total += adjustedAmount;
      } else if ((i + 1) <= 75) {
        distribution.t_51_75.percent += percentage;
        distribution.t_51_75.total += adjustedAmount;
      } else if ((i + 1) <= 100) {
        distribution.t_76_100.percent += percentage;
        distribution.t_76_100.total += adjustedAmount;
      } else {
        distribution.t_101plus.percent += percentage;
        distribution.t_101plus.total += adjustedAmount;
      }
      loop();
    }, function() {
      // Format percentages and totals
      ...
      distribution.t_101plus.percent = parseFloat(distribution.t_101plus.percent).toFixed(2);
      distribution.t_101plus.total = parseFloat(distribution.t_101plus.total).toFixed(8);
  
      // Ensure total doesn't exceed supply
      const top100Total = distribution.t_1_25.total + distribution.t_26_50.total + distribution.t_51_75.total + distribution.t_76_100.total;
      if (top100Total > stats.supply) {
        const excess = top100Total - stats.supply;
        distribution.t_1_25.total = (distribution.t_1_25.total - (excess * (distribution.t_1_25.total / top100Total))).toFixed(8);
        distribution.t_26_50.total = (distribution.t_26_50.total - (excess * (distribution.t_26_50.total / top100Total))).toFixed(8);
        distribution.t_51_75.total = (distribution.t_51_75.total - (excess * (distribution.t_51_75.total / top100Total))).toFixed(8);
        distribution.t_76_100.total = (distribution.t_76_100.total - (excess * (distribution.t_76_100.total / top100Total))).toFixed(8);
        distribution.t_1_25.percent = ((distribution.t_1_25.total / stats.supply) * 100).toFixed(2);
        distribution.t_26_50.percent = ((distribution.t_26_50.total / stats.supply) * 100).toFixed(2);
        distribution.t_51_75.percent = ((distribution.t_51_75.total / stats.supply) * 100).toFixed(2);
        distribution.t_76_100.percent = ((distribution.t_76_100.total / stats.supply) * 100).toFixed(2);
      }
